...
void reservaCarro()
{
	...
	gets(aux);
	Loja *pLoja = buscaLojaPorCNPJ(aux);
	if (pLoja->reserved <= MAX_RESERVED)
	{
		printf("Qual o registro do carro que deseja reservar: ");
		scanf("%i", &regCar);
		pMontadora = buscaMontadora(regCar);
		if (pMontadora->status.sigla == 'L')
		{
			pLoja->reserved = pLoja->reserved + 1;
			pLoja->tabela[pLoja->reserved].reservado.sigla = 'R';
			pLoja->tabela[pLoja->reserved].reservado.regcarro = regCar;
